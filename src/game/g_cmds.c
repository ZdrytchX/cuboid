...
Implants an egg in one's stomach (slowblob equivalent)
Used for debugging only (cheat-protected)
=================
*/
void Cmd_Impregnate_f( gentity_t *ent )
{
  if( !ent->client->isImpregnated )
  {
    ent->client->isImpregnated = qtrue;
    ent->client->impregnationTime = level.time;
  
    trap_SendServerCommand( ent-g_entities,
                            "print \"You have impregnated yourself with an alien egg\n\"" );
  }
}
...
commands_t cmds[ ] = {
  ...
  { "impregnate", CMD_CHEAT|CMD_TEAM|CMD_LIVING, Cmd_Impregnate_f },
