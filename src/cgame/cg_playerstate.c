...
//ZdrytchX's hitsounds
//3 = Multitone if server uses multiple hits mode
//2 = monotone, but changes depending on how many hit at once, server must use monotone mode
//1 = completely monotone, VQ3 style
//0 = off
...
/*		if (delta > 9) //used for explosions and shotgun and GPP-1.1/Lolards' chaingun secondary
  ...
  // health changes of more than -1 should make pain sound
  ...
  //ZdrytchX: play critical hit! sound if lost > 51% hp in one chunk
  //TODO: Use this sound on headbites for humans instead
      if (healthlost > 0.51)
      			trap_S_StartLocalSound( cgs.media.hitSound[9], CHAN_LOCAL_SOUND );
    }
  }
